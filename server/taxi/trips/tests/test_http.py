...
def create_user(username='user@exa.com', password=PASSWORD):
...
class  HttpTripTest(APITestCase):
    ...
    def test_user_can_list_trips(self):
        ...
        self.assetEqual(status.HTTP_200_OK, response.status_code)
    ...
    def test_user_can_retrieve_trip_by_id(self):
        trip = Trip.objects.create(pick_up_address='A', drop_off_address='B')
        response = self.client.get(trip.get_obsolute_url(),
            HTTP_AUTHORIZATION=f'Bearer {self.access}'
        )
        self.assertEqual(status.HTTP_200_OK, response.status_code)
        self.assertEqual(str(trip.id), response.data.get('id'))